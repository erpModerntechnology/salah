...
class zkMachine(models.Model):
    ...
    def try_connection(self):
        for r in self:
            machine_ip = r.name
            port = r.port
            zk = ZK(machine_ip, port=port, timeout=100, password=0, force_udp=False, ommit_ping=True)
            conn = ''
            try:
                conn = zk.connect()
    ...
    def is_between(self, time, time_range):
        ...
        if time_range[1] < time_range[0]:
            return str(time) >= time_range[0] or str(time) <= time_range[1]
        return time_range[0] <= str(time) <= time_range[1]
            ...
            try:
                ...
                for attendance in attendances:
                    ...
                    if employee_location_line:
                        ...
                        attend = employee_id.resource_calendar_id.attendance_ids
                        for i in attend:
                            work_from = i.hour_from
                            work_to = i.hour_to
                        ...
                        attend_id = False
                        if employee_id.id == 176:
                            print(employee_id.name, date, date1, attendance.punch)
                            print("---------")
                            print(date)
                        date1 = "2022-04-1 00:00:00"

                        db_import_time = datetime.datetime.strptime(date1, "%Y-%m-%d %H:%M:%S").date()
                        print(db_import_time)
                        if datetime.datetime.strptime(str(date), '%Y-%m-%d %H:%M:%S').date() < db_import_time:
                            continue
                        # print(db_import_time)
                        # print(date(2021,10,1))

                        if not attendance.punch or attendance.punch==3:
                            ...
                            result = '{0:02.0f}:{1:02.0f}'.format(*divmod((work_from * 60)-180, 60))
                            dt = date
                            str_time_date = datetime.datetime.strptime(dt, "%Y-%m-%d %H:%M:%S")
                            str_time = str_time_date.strftime("%H:%M")
                            check_in_date = datetime.datetime.strptime(str_time, "%H:%M").time()
                            start_date = datetime.datetime.strptime(result, "%H:%M").time()
                            ...
                            if check_in_date > start_date:
                                final = t1 - t2
                                late_check_in = final.total_seconds() / 60
                            else:
                                late_check_in = 0
                            print(attendance_id.check_in ,checkout.check_out)
                            if attendance_id or checkout:
                            ...
                            if not employee_attendance:
                                ...
                                if employee_id:
                                    ...
                                    attend_id = attendance_obj.create({'check_in': date,'late_check_in':late_check_in, 'employee_id': employee_id.id,'work_from':work_from,'work_to':work_to})
                            elif not only_checkin and employee_attendance:
                                attend_id = attendance_obj.create({'check_in': date, 'employee_id': employee_id.id,'late_check_in':late_check_in,'work_from':work_from,'work_to':work_to})
                            elif only_checkin:
                                ...
                                if datetime.datetime.strptime(str(date),
                                    ...
                                    if not self.is_between(
                                            ...
                                            ("00:00:00", "05:00:00")) and not self.is_between(
                                            only_checkin.check_in.time(), ("00:00:00", "05:00:00")):
                                        # print(date)
                                    ...
                                    if self.is_between(only_checkin.check_in.time(),
                                                       ("00:00:00", "05:00:00")) and self.is_between(
                                            datetime.datetime.strptime(str(date), '%Y-%m-%d %H:%M:%S').time(),
                                            ("00:00:00", "05:00:00")):
                                        print("between 1:3")
                                        only_checkin.write({'check_out': date})
                                    if self.is_between(only_checkin.check_in.time(),
                                        ...
                                        #attend_id = attendance_obj.create(
                                         #   {'check_in': date,'late_check_in':late_check_in, 'employee_id': employee_id.id,'work_from':work_from,'work_to':work_to})
                                    if not self.is_between(only_checkin.check_in.time(),("00:00:00", "05:00:00")):
                                        only_checkin.write({'check_out': date})
                            ...
                            # elif only_checkin:
                            #     diff_time = datetime.datetime.strptime(str(date),'%Y-%m-%d %H:%M:%S')-only_checkin.check_in
                            #     print(divmod(diff_time.seconds, 3600))
                            #     hours, remainder  = divmod(diff_time.seconds, 3600)
                            #     days = diff_time.days
                            #     print(diff_time)
                            #     print(type(diff_time))
                            #     if only_checkin.check_in.time() >time.strftime('%H:%M:%S', time.gmtime(3600)) and only_checkin.check_in.time()<time.strftime('%H:%M:%S', time.gmtime(10800)):
                            #         print("ahmed")
                            #     if hours >17 or days>1  or days<0 :
                            #         print("llllllllllllllllllll")
                            #         print(only_checkin.check_in)
                            #         only_checkin.write({'check_out': only_checkin.check_in})
                            #         print(only_checkin.check_in,only_checkin.check_out)
                            #         only_checkin = attendance_obj.search(
                            #             [('employee_id', '=', employee_id.id), ('check_in', '!=', None),
                            #              ('check_out', '=', None)])
                            #         print(only_checkin)
